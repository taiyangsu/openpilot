...
FrogPilotControlsPanel::FrogPilotControlsPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> controlToggles {
    {"AlwaysOnLateral", tr("Always on Lateral"), tr("在使用刹车或油门踏板时保持openpilot的横向控制。\n\n仅通过'巡航控制'按钮停用。"), "../frogpilot/assets/toggle_icons/icon_always_on_lateral.png"},
    {"AlwaysOnLateralMain", tr("Enable On Cruise Main"), tr("通过点击'巡航控制'按钮启用'Always On Lateral'，无需先启用openpilot。"), ""},
    {"PauseAOLOnBrake", tr("Pause On Brake Below"), tr("当刹车踏板在设定速度以下被按下时暂停'Always On Lateral'。"), ""},
    {"HideAOLStatusBar", tr("Hide the Status Bar"), tr("不使用'Always On Lateral'的状态栏。"), ""},

    {"ConditionalExperimental", tr("Conditional Experimental Mode"), tr("在预定义条件下自动切换到'实验模式'。"), "../frogpilot/assets/toggle_icons/icon_conditional.png"},
    {"CECurves", tr("Curve Detected Ahead"), tr("检测到曲线时切换到'实验模式'。"), ""},
    {"CENavigation", tr("Navigation Based"), tr("基于导航数据切换到'实验模式'。（例如：交叉路口、停车标志、即将转弯等）"), ""},
    {"CESlowerLead", tr("Slower/Stopped Lead Detected Ahead"), tr("检测到前方有较慢或停止的前车时切换到'实验模式'。"), ""},
    {"CEStopLights", tr("Stop Lights and Stop Signs"), tr("检测到红绿灯或停车标志时切换到'实验模式'。"), ""},
    {"CESignal", tr("Turn Signal When Below Highway Speeds"), tr("在高速公路速度以下使用转向灯时切换到'实验模式'以帮助转弯。"), ""},
    {"HideCEMStatusBar", tr("Hide the Status Bar"), tr("不使用'条件实验模式'的状态栏。"), ""},

    {"DeviceManagement", tr("Device Management"), tr("根据个人喜好调整设备的行为。"), "../frogpilot/assets/toggle_icons/icon_device.png"},
    {"DeviceShutdown", tr("Device Shutdown Timer"), tr("配置设备在离开道路后多快关闭。"), ""},
    {"NoLogging", tr("Disable Logging"), tr("关闭所有数据跟踪以增强隐私或减少热负荷。"), ""},
    {"NoUploads", tr("Disable Uploads"), tr("关闭所有数据上传到comma的服务器。"), ""},
    {"IncreaseThermalLimits", tr("Increase Thermal Safety Limit"), tr("允许设备在高于comma推荐的热限温度下运行。"), ""},
    {"LowVoltageShutdown", tr("Low Voltage Shutdown Threshold"), tr("当电池达到特定电压水平时自动关闭设备以防止电池损坏。"), ""},
    {"OfflineMode", tr("Offline Mode"), tr("允许设备无限期离线。"), ""},

    {"DrivingPersonalities", tr("Driving Personalities"), tr("管理comma的'个性化配置文件'的驾驶行为。"), "../frogpilot/assets/toggle_icons/icon_personality.png"},
    {"CustomPersonalities", tr("Customize Personalities"), tr("根据您的驾驶风格自定义驾驶个性化配置文件。"), ""},
    {"TrafficPersonalityProfile", tr("Traffic Personality"), tr("自定义'Traffic'个性化配置文件。"), "../frogpilot/assets/other_images/traffic.png"},
    {"TrafficFollow", tr("Following Distance"), tr("设置使用'Traffic Mode'时的最小跟车距离。您的跟车距离将在0到%1之间动态调整，介于此距离和'Aggressive'配置文件的跟车距离之间。\n\n例如：\n\nTraffic Mode: 0.5秒\nAggressive: 1.0秒\n\n0%2 = 0.5秒\n%3 = 0.75秒\n%1 = 1.0秒"), ""},
    {"TrafficJerkAcceleration", tr("Acceleration/Deceleration Response Offset"), tr("自定义使用'Traffic Mode'时的加速响应率。"), ""},
    {"TrafficJerkSpeed", tr("Speed Control Response Offset"), tr("自定义使用'Traffic Mode'时保持速度（包括制动）的响应率。"), ""},
    {"ResetTrafficPersonality", tr("Reset Settings"), tr("将'Traffic Mode'个性化配置的值重置为默认值。"), ""},
    {"AggressivePersonalityProfile", tr("Aggressive Personality"), tr("自定义'Aggressive'个性化配置文件。"), "../frogpilot/assets/other_images/aggressive.png"},
    {"AggressiveFollow", tr("Following Distance"), tr("设置'Aggressive'个性化配置的跟车距离。表示跟随前车的秒数。\n\n默认值：1.25秒。"), ""},
    {"AggressiveJerkAcceleration", tr("Acceleration/Deceleration Response Offset"), tr("自定义使用'Aggressive'个性化配置时的加速响应率。"), ""},
    {"AggressiveJerkSpeed", tr("Speed Control Response Offset"), tr("自定义使用'Aggressive'个性化配置时保持速度（包括制动）的响应率。"), ""},
    {"ResetAggressivePersonality", tr("Reset Settings"), tr("将'Aggressive'个性化配置的值重置为默认值。"), ""},
    {"StandardPersonalityProfile", tr("Standard Personality"), tr("自定义'Standard'个性化配置文件。"), "../frogpilot/assets/other_images/standard.png"},
    {"StandardFollow", tr("Following Distance"), tr("设置'Standard'个性化配置的跟车距离。表示跟随前车的秒数。\n\n默认值：1.45秒。"), ""},
    {"StandardJerkAcceleration", tr("Acceleration/Deceleration Response Offset"), tr("自定义使用'Standard'个性化配置时的加速响应率。"), ""},
    {"StandardJerkSpeed", tr("Speed Control Response Offset"), tr("自定义使用'Standard'个性化配置时保持速度（包括制动）的响应率。"), ""},
    {"ResetStandardPersonality", tr("Reset Settings"), tr("将'Standard'个性化配置的值重置为默认值。"), ""},
    {"RelaxedPersonalityProfile", tr("Relaxed Personality"), tr("自定义'Relaxed'个性化配置文件。"), "../frogpilot/assets/other_images/relaxed.png"},
    {"RelaxedFollow", tr("Following Distance"), tr("设置'Relaxed'个性化配置的跟车距离。表示跟随前车的秒数。\n\n默认值：1.75秒。"), ""},
    {"RelaxedJerkAcceleration", tr("Acceleration/Deceleration Response Offset"), tr("自定义使用'Relaxed'个性化配置时的加速响应率。"), ""},
    {"RelaxedJerkSpeed", tr("Speed Control Response Offset"), tr("自定义使用'Relaxed'个性化配置时保持速度（包括制动）的响应率。"), ""},
    {"ResetRelaxedPersonality", tr("Reset Settings"), tr("将'Relaxed'个性化配置的值重置为默认值。"), ""},
    {"OnroadDistanceButton", tr("Onroad Distance Button"), tr("通过onroad UI模拟一个距离按钮来控制个性化配置、'实验模式'和'Traffic Mode'。"), ""},
